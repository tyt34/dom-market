...
- - Геопозиция объекта генерируется случайным образом в пределах окрестностей города Волгоград
- - Для отображения карты используется *Google Maps* через `iframe`
- - Режим отображения карточек (`row`/`column`) сохраняется в `localStorage` с использованием ключей, зависящих от категории, что обеспечивает независимое хранение *UI*-настроек для разных разделов каталога
...
- Виджеты и дополнительные *UI*-инструменты
- - В приложение добавлен *drag-and-drop*-виджет настроек, реализованный с использованием *Framer Motion*
- - Открытие виджета происходит через пункт меню «Настройки»
- - Реализована возможность изменения фонового изображения интерфейса и возвращение изначальной картинки
- - Загруженное изображение применяется как фон для компонента фильтров
- - Поддерживается сохранение выбранного изображения в *IndexedDB*, что позволяет восстанавливать фон после перезагрузки страницы
- Адаптивность
